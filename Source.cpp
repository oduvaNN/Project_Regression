...
namespace Project2
{
	...
	void regression_model(double &a1, double &a0,int &type)//функція знаходження справжніх коефіцієнтів а0 і а1
	{
		...
		mod_sort(models);
		//7.Знаходження значень
		for (int i = 0; i < CountOfModels; i++)
		{
			//7.1 Перетворена матриця Х
			double** matrix_x = create_matrix_x(models[i].valx, colsCount);
			//7.2 Транспонована матриця Х (Хт)
			double** matrix_t = transpose_matrix(matrix_x, colsCount);
			double* matrix_y = models[i].valy;
			//7.3 Матриця множення Хт на Х
			double** mult_matrix = multiply_matrix(matrix_t, 2, colsCount, matrix_x, colsCount, 2);
			//7.4 Обернена матриця 7.3
			double** inversed_matrix = inversion(mult_matrix, 2);
			//7.5 Обернену матрицю помножити на Хт
			mult_matrix = multiply_matrix(inversed_matrix, 2, 2, matrix_t, 2, colsCount);
			//7.6 Результат пункту 7.5 помножили на матрицю У та отримали матрицю значень а0, а1
			double* matrixA = multiply_matrix(mult_matrix, 2, colsCount, matrix_y, colsCount);
			//7.7 Знаходження залишкової дисперсії
			double res_dispersion = residual_dispersion(colsCount, models[i].valx, models[i].valy, matrixA);
			//7.8 Стандартна похибка вільного члена а0
			double S0 = standart_mistake_S0(res_dispersion, models[i].DispX, colsCount);
			//7.9 Стандартна похибка вільного члена а1
			double S1 = standart_mistake_S1(res_dispersion, models[i].DispX, colsCount, models[i].averagex);
			//7.10 Довірчий інтервал а0
			double interval_a0[2];
			//7.11 Знаходження довірчого інтервалу а0
			find_interval(interval_a0, matrixA, S0, 0);
			//7.12  Довірчий інтервал а1
			double interval_a1[2];
			//7.13 Знаходження довірчого інтервалу а1
			find_interval(interval_a1, matrixA, S1, 1);
			//8.Перевірка інтервалу
			if (Sign(interval_a0[0]) == Sign(interval_a0[1]) && (Sign(interval_a1[0]) == Sign(interval_a1[1])))
			{
				//9.1 Знаходження справжнього значення а1
				models[i].a1 = round((1 / matrixA[0]) * 100000) / 100000;
				//9.2 Знаходження справжнього значення а0
				models[i].a0 = round((models[i].a1 * matrixA[1]) * 100000) / 100000;
				//9.3 Присвоєння даних змінним а0, а1, type
				a1 = models[i].a1;
				a0 = models[i].a0;
				type = models[i].type;
				break;
			...
			for (auto i : models)
				i.Distruct();
			//10. Видалення всіх масивів з пам'яті
			delete_all(matrix_x, matrix_t, mult_matrix, matrix_y, inversed_matrix, matrixA, colsCount);
		}
		delete[] matrixx;
		delete[] matrixy;
